...
static const param_meta_t g_params[] = {
    ...
    {"scan_enable", "Scan Enable", PARAM_BOOL, 0.0f, 1.0f, offsetof(grn_params_t, scan_enable)},
...
static void init_default_params(grain_instance_t *inst) {
    ...
    inst->params.amp_attack_ms = 10.0f;
    inst->params.amp_decay_ms = 200.0f;
    inst->params.amp_sustain = 0.80f;
    inst->params.amp_release_ms = 250.0f;
        ...
        for (int i = 0; i < PARAM_COUNT; i++) {
            ...
            if (strcmp(g_params[i].key, "size_ms") == 0) {
                ...
                offset += snprintf(buf + offset, buf_len - offset,
                                   ...
                                   g_params[i].min_val,
                                   g_params[i].max_val);
            } else if (strcmp(g_params[i].key, "jitter") == 0) {
                offset += snprintf(buf + offset, buf_len - offset,
                                   "{\"key\":\"%s\",\"name\":\"%s\",\"type\":\"%s\",\"min\":%g,\"max\":%g,\"step\":0.001}",
                                   g_params[i].key,
                                   g_params[i].name,
    ...
    if (strcmp(key, "ui_hierarchy") == 0) {
        const char *hier = "{"
            "\"levels\":{"
                ...
                "\"pitch_voice\":{"
                    "\"name\":\"Pitch / Voice\","
                    "\"knobs\":[\"pitch_semi\",\"fine_cents\",\"keytrack\",\"play_mode\",\"polyphony\",\"portamento_ms\",\"spread\"],"
                    "\"params\":[\"pitch_semi\",\"fine_cents\",\"keytrack\",\"play_mode\",\"polyphony\",\"portamento_ms\",\"spread\"]"
                "},"
                "\"window_tone\":{"
                    "\"name\":\"Window / Tone\","
                    "\"knobs\":[\"window_type\",\"window_shape\",\"grain_gain\",\"quality\",\"trigger_mode\"],"
                    "\"params\":[\"window_type\",\"window_shape\",\"grain_gain\",\"quality\",\"trigger_mode\",\"amp_attack_ms\",\"amp_decay_ms\",\"amp_sustain\",\"amp_release_ms\"]"
                "},"
                "\"scan_menu\":{"
                    "\"name\":\"Scan\","
                    "\"knobs\":[\"scan_enable\",\"scan\",\"scan_end_mode\"],"
                    "\"params\":[\"scan_enable\",\"scan\",\"scan_end_mode\"]"
                "}"
            "}"
        "}";
